...
## 2025-02-23 - Missing Role-Based Access Control (RBAC) on Device and School Endpoints
**Vulnerability:** Critical global device provisioning endpoints (e.g., `/api/devices`, `/api/devices/:id`) and global school administration endpoints (e.g., `GET /api/schools`) were completely missing authorization checks.
**Learning:** While the global `authenticate` middleware verified token validity, it did not enforce any role-based restrictions. Consequently, any authenticated user with a valid JWT (like a PARENT or DRIVER) could list all schools, list all devices, provision new devices, or modify/delete existing devices across the entire system.
**Prevention:** Always verify that sensitive, global administration endpoints have explicit role-based access checks (e.g., `authorizeRoles('SUPER_ADMIN')`) applied at the route definition or using an `app.use` prefix. Never assume authentication implies authorization.
