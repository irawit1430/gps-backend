...
## 2024-05-18 - Fix password leak in driver endpoint
**Learning:** `prisma.user.create()` returns all fields by default, including sensitive ones like `password`.
**Action:** Always include a `select` statement when creating or querying users to prevent inadvertently returning password hashes to the client.
