...
// ⚡ Bolt: Added in-memory cache to prevent frequent DB queries on every TCP hardware ping
const tcpBusCache = new Map();
      ...
      while ((lastIndex = bufferAcc.indexOf('*')) !== -1) {
        ...
        try {
          ...
          if (parsed && parsed.imei) {
            ...
            let bus = null;
            const cached = tcpBusCache.get(parsed.imei);
            ...
            if (cached && (Date.now() - cached.timestamp < CACHE_TTL_MS)) {
              ...
              if (bus) {
                tcpBusCache.set(parsed.imei, { data: bus, timestamp: Date.now() });
              }
            }
